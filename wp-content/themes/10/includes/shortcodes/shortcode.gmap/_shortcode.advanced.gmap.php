...
function cloudfw_javascript_gmap(){
	...
	if ( ! empty( $api_key ) ) {
		wp_register_script ('theme-gmap-api', $schema . 'maps.googleapis.com/maps/api/js?v=3.25&key='. $api_key .'&language=' . $language_code,  array(), NULL );
	} else {
		wp_register_script ('theme-gmap-api', $schema . 'maps.googleapis.com/maps/api/js?v=3.25&language=' . $language_code,  array(), NULL );
	}
