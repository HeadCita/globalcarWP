...
$map  -> option  ( 'texts' )
    ...
    -> sub     ( 'next', __('Next','cloudfw') )
    -> sub     ( 'previous', __('Previous','cloudfw') )
    -> sub     ( 'search', __('Search','cloudfw') )
    -> sub     ( 'mobile_navigation',  __('Navigation','cloudfw') )
    -> sub     ( 'close',  __('Close','cloudfw') )

    -> sub     ( '404_title', __('<strong>404</strong> Not Found','cloudfw') )
    -> sub     ( '404_text',  __('The page you try to see cannot be found','cloudfw') )
    
    -> sub     ( 'enter_a_term_to_search',  __('Please enter a term to search','cloudfw') )
    -> sub     ( 'nothing_found',  __('Nothing Found','cloudfw') )
    -> sub     ( 'no_posts_matched',  __('Sorry, no posts matched your criteria','cloudfw') )
    -> sub     ( 'search_titles', __('Search results for "%s"','cloudfw') )
    -> sub     ( 'search_result_count', __('There are <strong>%2$s</strong> result(s) with the term "<strong>%1$s</strong>"','cloudfw') )
    ...
    -> sub     ( 'related_posts', __('<strong>Related</strong> Posts', 'cloudfw') )
    -> sub     ( 'by_author', __('by %1$s','cloudfw') )
    -> sub     ( 'meta_tags', __('Tags: %s','cloudfw') )
    ...
    -> sub     ( 'portfolio_category',  __('Portfolio Category: <strong>%s</strong>','cloudfw') )
    -> sub     ( 'portfolio_filter',  __('Portfolio Filter: <strong>%s</strong>','cloudfw') )
    ...
    -> sub     ( 'wc.catalog.display',  __('Display','cloudfw') )
    -> sub     ( 'wc.catalog.d_products',  __('%d products','cloudfw') )
    -> sub     ( 'wc.catalog.s_items',  __('%s Item(s)','cloudfw') )
    -> sub     ( 'wc.catalog.quick_view',  __('Quick View','cloudfw') )
