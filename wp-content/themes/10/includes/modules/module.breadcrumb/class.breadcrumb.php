...
function cloudfw_breadcrumb( $args = array() ) {
	...
	if ( !empty( $trail ) && is_array( $trail ) ) {
		...
		$breadcrumb = '<' . tag_escape( $args['container'] ) . ' class="ui-bc ui-breadcrumbs breadcrumbs" itemscope itemtype="http://schema.org/BreadcrumbList">';
		...
		foreach ($trail as $number => $item) {
			...
			if ( is_array($item) ) {
				$extra_class = ''; 
				if ( $number === 0 ) {
					$extra_class .= ' ui-bc-first';
				}

				if ( $number === ($trail_count - 1) ) {
					$extra_class .= ' ui-bc-last';
				}

				$extra_class .= ! empty( $item['class'] ) ? ' ' .$item['class'] : '';
				$item_prop = false;
				if ( ! isset($item['itemprop']) || $item['itemprop'] !== false ) {
					$item_prop = true;
				}


				if ( ! empty( $item['link'] ) ) {
					$breadcrumb .= '<span class="ui-bc-item'. $extra_class .'"';
					if ( $item_prop ) {
						$breadcrumb .= ' itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"';
					}
					$breadcrumb .= '>';
						$breadcrumb .= '<a href="'. esc_url($item['link']) .'"';
						if ( $item_prop ) {
							$breadcrumb .= ' itemprop="item"';
						}
						if ( !empty($item['rel']) ) {
							$breadcrumb .= ' rel="'. esc_attr( $item['rel'] ) .'"';
						}
						$breadcrumb .= '><span';
						if ( $item_prop ) {
							$breadcrumb .= ' itemprop="name"';
						}
						$breadcrumb .= '>'. $item['title'] .'</span></a>';
					$breadcrumb .= '</span>';
				} else {
					$breadcrumb .= '<span class="ui-bc-item'. $extra_class .'">';
...
function cloudfw_breadcrumb_get_items( $args = array() ) {
	...
	if ( !is_front_page() && $args['show_home'] ) {
		...
		if ( is_multisite() && true === $args['network'] ) {
			$trail[] = array( 'link' => network_home_url(), 'title' => $args['show_home'] );
			$trail[] = array( 'link' => home_url(), 'title' => get_bloginfo( 'name' ), 'class' => '', 'rel' => 'home', 'itemprop' => false );
		} else {
			$trail[] = array( 'link' => home_url(), 'title' => $args['show_home'], 'class' => '', 'rel' => 'home', 'itemprop' => false, 'alt' => get_bloginfo( 'name' ) );
		}
	}
	...
	elseif ( is_front_page() ) {
		...
		if ( !is_paged() && $args['show_home'] && $args['front_page'] ) {
			...
			if ( is_multisite() && true === $args['network'] ) {
				$trail[] = array( 'url' => network_home_url(), 'title' => $args['show_home'] );
				$trail[] = array( 'title' => get_bloginfo( 'name' ) );
			} else {
				$trail[] = array( 'title' => $args['show_home'] );
			}
		}
	...
	elseif ( is_home() ) {
		...
		if ( is_paged() )
			$trail[] = array( 'link' => get_permalink( $home_page->ID ), 'title' => get_the_title( $home_page->ID ) );
		else
			$trail[] = array( 'title' => get_the_title( $home_page->ID ) );
	}
	...
	elseif ( is_singular() ) {
			...
			if ( 0 < $parent_id ) {
				...
				elseif ( 'page' !== $parent_post_type ) {
					...
					if ( !empty( $parent_post_type_object->has_archive ) ) {
						...
						$trail[] = array( 'link' => get_post_type_archive_link( $parent_post_type ), 'title' => $label );
		...
		elseif ( 'page' !== $post_type ) {
			...
			if ( !empty( $post_type_object->has_archive ) ) {
				...
				if ( apply_filters( "cloudfw_breadcrumbs_is_singular_" . $post_type_object->name, true ) ) {
					...
					$trail[] = array( 'link' => get_post_type_archive_link( $post_type ), 'title' => $label );
		...
		if ( 1 < get_query_var( 'page' ) && !empty( $post_title ) )
			$trail[] = array( 'link' => get_permalink( $post_id ), 'title' => $post_title );


		elseif ( !empty( $post_title ) )
			$trail[] = array( 'title' => $post_title );
	...
	elseif ( is_archive() && !is_search() ) {
		...
		if ( is_tax() || is_category() || is_tag() ) {
				...
				if ( !empty( $post_types ) ) {
					...
					foreach ( $post_types as $post_type_object ) {
						...
						if ( $taxonomy->rewrite['slug'] === $post_type_object->has_archive ) {
							...
							$trail[] = array( 'link' => get_post_type_archive_link( $post_type_object->name ), 'title' => $label );
			...
			if ( is_paged() )
				$trail[] = array( 'link' => get_term_link( $term, $term->taxonomy ), 'title' => single_term_title( '', false ) );
			else
				$trail[] = array( 'title' => single_term_title( '', false ) );
		}
		...
		elseif ( is_post_type_archive() ) {
			...
			if ( apply_filters( "cloudfw_breadcrumbs_is_post_type_archive_" . $post_type_object->name, true ) ) {
				...
				if ( is_paged() )
					$trail[] = array( 'link' => get_post_type_archive_link( $post_type_object->name ), 'title' => post_type_archive_title( '', false ) );
				else
					$trail[] = array( 'title' => post_type_archive_title( '', false ) );
			}			
		...
		elseif ( is_author() ) {
			...
			if ( is_paged() )
				$trail[] = array( 'link' => get_author_posts_url( $user_id ), 'title' => get_the_author_meta( 'display_name', $user_id ) );
			else
				$trail[] = array( 'title' => get_the_author_meta( 'display_name', $user_id ) );
		}
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail[] = array( 'title' => get_the_time( __( 'g:i a', 'cloudfw' ) ) );


			elseif ( get_query_var( 'minute' ) )
				$trail[] = array( 'title' => sprintf( __( 'Minute %1$s', 'cloudfw' ), get_the_time( __( 'i', 'cloudfw' ) ) ) );

			elseif ( get_query_var( 'hour' ) )
				$trail[] = array( 'title' => get_the_time( __( 'g a', 'cloudfw' ) ) );
		}
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				$trail[] = array( 'link' => get_year_link( get_the_time( 'Y' ) ), 'title' => get_the_time( __( 'Y', 'cloudfw' ) ) );
				$trail[] = array( 'link' => get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ), 'title' => get_the_time( __( 'F', 'cloudfw' ) ) );
				$trail[] = array( 'link' => get_day_link( get_the_time( 'Y' ), get_the_time( 'm' ), get_the_time( 'd' ) ), 'title' => get_the_time( __( 'd', 'cloudfw' ) ) );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = array( 'link' => get_year_link( get_the_time( 'Y' ) ), 'title' => get_the_time( esc_attr__( 'Y', 'cloudfw' ) ) );
			...
			elseif ( is_month() ) {
				$trail[] = array( 'link' =>  get_year_link( get_the_time( 'Y' ) ), 'title' => get_the_time( esc_attr__( 'Y', 'cloudfw' ) ) );
				$trail[] = array( 'link' =>  get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ), 'title' => get_the_time( esc_attr__( 'F', 'cloudfw' ) ) );
			}
	...
	elseif ( is_search() ) {
		...
		if ( is_paged() )
			$trail[] = array( 'link' => get_search_link(), 'title' => sprintf( esc_attr(cloudfw_translate('search_titles')), esc_attr( get_search_query() ) ) );
		else
			$trail[] = array( 'title' => sprintf( esc_attr(cloudfw_translate('search_titles')), esc_attr( get_search_query() ) ) );
	}

	/* If viewing a 404 error page. */
	elseif ( is_404() ) {
		$trail[] = array( 'title' => strip_tags(__t( cloudfw_get_option( 'texts', '404_title'), 'cloudfw_wpml', 'texts_404_title' )) );
	}
...
function cloudfw_breadcrumb_get_bbpress_items( $args = array() ) {
	...
	if ( !empty( $post_type_object->has_archive ) && !bbp_is_forum_archive() )
		$trail[] = array( 'link'=> get_post_type_archive_link( bbp_get_forum_post_type() ), 'title' => bbp_get_forum_archive_title() );

	/* If viewing the forum root/archive. */
	if ( bbp_is_forum_archive() ) {
		$trail[] = array( 'title' => bbp_get_forum_archive_title() );
	}

	/* If viewing the topics archive. */
	elseif ( bbp_is_topic_archive() ) {
		$trail[] = array( 'title' => bbp_get_topic_archive_title() );
	}

	/* If viewing a topic tag archive. */
	elseif ( bbp_is_topic_tag() ) {
		$trail[] = array( 'title' => bbp_get_topic_tag_name() );
	}

	/* If viewing a topic tag edit page. */
	elseif ( bbp_is_topic_tag_edit() ) {
		$trail[] = array( 'link' => bbp_get_topic_tag_link(), 'title' => bbp_get_topic_tag_name() );
		$trail[] = array( 'title' => __( 'Edit', 'cloudfw' ) );
	}

	/* If viewing a "view" page. */
	elseif ( bbp_is_single_view() ) {
		$trail[] = array( 'title' => bbp_get_view_title() );
	}
	...
	elseif ( bbp_is_single_topic() ) {
		...
		if ( bbp_is_topic_split() || bbp_is_topic_merge() || bbp_is_topic_edit() )
			$trail[] = array( 'link' => bbp_get_topic_permalink( $topic_id ), 'title' => bbp_get_topic_title( $topic_id ) );
		else
			$trail[] = array( 'title' => bbp_get_topic_title( $topic_id ) );

		/* If viewing a topic split page. */
		if ( bbp_is_topic_split() )
			$trail[] = array( 'title' => __( 'Split', 'cloudfw' ) );

		/* If viewing a topic merge page. */
		elseif ( bbp_is_topic_merge() )
			$trail[] = array( 'title' => __( 'Merge', 'cloudfw' ) );
	...
	elseif ( bbp_is_single_reply() ) {
		...
		if ( bbp_is_reply_edit() ) {
			$trail[] = array( 'link' => bbp_get_reply_url( $reply_id ), 'title' => bbp_get_reply_title( $reply_id ) );
			$trail[] = array( 'title' => __( 'Edit', 'cloudfw' ) );

		} else {
			$trail[] = array( 'title' => bbp_get_reply_title( $reply_id ) );
		}
	...
	elseif ( bbp_is_single_forum() ) {
		...
		$trail[] = array( 'title' => bbp_get_forum_title( $forum_id ) );
	}
	...
	elseif ( bbp_is_single_user() || bbp_is_single_user_edit() ) {
		...
		if ( bbp_is_single_user_edit() ) {
			$trail[] = array( 'link' => bbp_get_user_profile_url(), 'title' => bbp_get_displayed_user_field( 'display_name' ) );
			$trail[] = array( 'title' => __( 'Edit', 'cloudfw' ) );
		} else {
			$trail[] = array( 'title' => bbp_get_displayed_user_field( 'display_name' ) );
		}
	}
...
if ( ! function_exists( 'cloudfw_breadcrumb_map_rewrite_tags' ) ) {
...
function cloudfw_breadcrumb_map_rewrite_tags( $post_id = '', $path = '', $args = array() ) {
	...
	if ( is_array( $matches ) ) {
		...
		foreach ( $matches as $match ) {
			...
			if ( '%year%' == $tag )
				$trail[] = array( 'link' => get_year_link( get_the_time( 'Y', $post_id ) ), 'title' =>  get_the_time( __( 'Y', 'cloudfw' ), $post_id ) );

			/* If using the %monthnum% tag, add a link to the monthly archive. */
			elseif ( '%monthnum%' == $tag )
				$trail[] = array( 'link' => get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ), 'title' =>  get_the_time( __( 'F', 'cloudfw' ), $post_id ), 'alt' => get_the_time( esc_attr__( 'F Y', 'cloudfw' ), $post_id ) );

			/* If using the %day% tag, add a link to the daily archive. */
			elseif ( '%day%' == $tag )
				$trail[] = array( 'link' => get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ), 'title' =>  get_the_time( __( 'd', 'cloudfw' ), $post_id ), 'alt' => get_the_time( esc_attr__( 'F j, Y', 'cloudfw' ), $post_id ) );

			/* If using the %author% tag, add a link to the post author archive. */
			elseif ( '%author%' == $tag )
				$trail[] = array( 'link' => get_author_posts_url( $post->post_author ), 'title' =>  get_the_author_meta( 'display_name', $post->post_author ) );
			...
			elseif ( '%category%' == $tag && 'category' !== $args["singular_{$post->post_type}_taxonomy"] ) {
				...
				if ( $terms ) {
					...
					$trail[] = array( 'link' => get_term_link( $term, 'category' ), 'title' =>  $term->name );
				}
			}
		}
	}
...
function cloudfw_breadcrumb_get_parents( $post_id = '', $path = '' ) {
	...
	while ( $post_id ) {
		...
		if ( 
			...
			$parents[] = array( 'link' => get_permalink( $post_id ), 'title' => get_the_title( $post_id ), 'class' => '' );
...
function cloudfw_breadcrumb_get_term_parents( $parent_id = '', $taxonomy = '' ) {
	...
	while ( $parent_id ) {
		...
		$parents[] = array( 'link' => get_term_link( $parent, $taxonomy ), 'title' => $parent->name, 'class' => '' );
