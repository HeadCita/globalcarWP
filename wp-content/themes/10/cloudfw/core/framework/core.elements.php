...
function cloudfw_help_dialog($id = NULL, $title = 'Help', $message = NULL, $float = 'none', $size = 's', $img = NULL, $video = NULL, $echo = true){
	...
	$content = ' <a href="#" class="cloudfw-help" rel="mb_' . $id . '" width="' . $size . '" style="float:' . $float . ';" title="' . $title . '" ><img src="' . TMP_ADMIN_GUI . '/' . $icon . '" /></a> <span id="mb_' . $id . '" style="display:none;height: auto;"> <div style="padding:0 20px;"> ' . $img . ' ' . $video . ' ' . $message . ' </div> <div class="clear"></div> </span>';
	if ($echo) {
		echo $content;
	}
		return $content;
...
function admin_create_color_selection($id, $preview = NULL, $value = NULL, $default_value = NULL, $class = NULL, $float = "left"){
	echo '<div style="float:' . $float . ';"><input type="text" size="6" id="' . $id . '" name="' . $id . '" autocomplete="off" value="', ($value) ? $value : $default_value, '"', ($preview) ? 'title="' . $id . '_ex_3"' : '', ' class="cloudfw-ui-color input', $class ? ' ' . $class : '', '" /></div>', ($preview) ? '<label for="' . $id . '" class="color-live" id="' . $id . '_ex_3"></label>' : '', '<div class="clear"></div>';
}
...
function admin_create_color_selection_v2($args = array()){
	...
	echo ' <div class="cloudfw-ui-color wrap-color' . _if(empty($value), ' empty') . '' . _if(($class), ' ' . $class) . '"> ';
			...
			echo ' <input type="text" id="' . $id . '" name="' . $name . '" autocomplete="off" value="' . $value . '" size="6" class="cloudfw-ui-color-input"';
			if( isset($reset) )
...
function admin_create_gradient_selection($args = array()){
	...
	echo ' <div class="cloudfw-ui-color wrap-color ui-gradient-selector' . _if(empty($start_color), ' empty') . '' . _if(($class), ' ' . $class) . '"> ';
		...
		echo '<div class="cloudfw-ui-color-handler start-color">';
			...
			echo ' <input type="text" id="' . $id . '" name="' . $name . '" autocomplete="off" value="' . $start_color . '" size="6" class="cloudfw-ui-color-input input-color-start"';
			if( isset($reset) )
				echo ' data-reset="'. $reset .'"';
			echo ' /> ';
		echo ' </div> ';
		...
		echo '<div class="cloudfw-ui-color-handler end-color">';
			...
			echo ' <input type="text" id="' . $id . '" name="' . $name . '" autocomplete="off" value="' . $end_color . '" size="6" class="cloudfw-ui-color-input input-color-stop"';
			if( isset($reset) )
				echo ' data-reset="'. $reset .'"';
			echo ' /> ';
		echo ' </div> ';
...
function admin_create_icon_selection($args = array()){
	...
	echo '<div class="wrap-icons' . _if(empty($value), ' empty')  . '' . _if(($class), ' ' . $class) . '"'.  _if($allow_custom, ' data-allow-customization="true"') .'>';
		...
		echo '<div class="cloudfw-ui-icon-select-menu">';
			...
			echo '<a href="javascript:;" class="font-icon-handler">'. __('Vector icon library','cloudfw') .'</a>';
			echo '<a href="javascript:;" class="library-icon-handler">'. __('Image icon library','cloudfw') .'</a>';
...
function admin_create_selectlist( $options = array() ) {
	...
	if ( $multiple ) {
		$items = cloudfw_array_sort_by_array($items, $value);
	}

	if ( (!isset($items[0]) || !is_array($items[0])) && count( $items ) > 0 ) {
		$re_items = array();
	...
	if ($type !== "select"):
		...
		echo '<select id="' . $id .'" name="' . $name . '" class="' . $main_select_class . '" autocomplete="off" style="'._if($width, 'width: '.$width.'px;')._if($height, 'height: '.$height.'px !important;').'"'._if($multiple, ' multiple="multiple"') . _if(isset($reset), ' data-reset="'. $reset .'"').' title="'. __('Please select','cloudfw') .'">';
		foreach ((array) $items as $item => $it) {
			$list_i++;
