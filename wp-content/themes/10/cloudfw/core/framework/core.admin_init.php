...
function cloudfw_setup_menu(){
	...
	if ( cloudfw_user_can_see( 'framework', 'administrator', 'theme_control_panel' ) ) {
		add_menu_page($menu_title . ' - ' . __('Settings', 'cloudfw') . ' - CloudFW', $menu_title . _if($last_version, '<span class="update-plugins cloudfw-notify"><span class="plugin-count">'.$last_version.'</span></span>'), 'edit_pages', $cloudfw_setting_slug, 'cloudfw_setup_setting', '', 701);
	...
	$cap_slider_manager = cloudfw_get_option( 'caps', 'slider_manager' );
	$show_slider_manager = false;
	if ( current_user_can( $cap_slider_manager ) ) {
		$show_slider_manager = true;
	} else {
		$show_slider_manager = cloudfw_user_can_see( 'slider_manager', 'administrator', 'theme_slider_manager' );
	}

	if ( $show_slider_manager ) {
		add_menu_page($menu_title . ' - Slider Manager - CloudFW', __("Slider Manager", "cloudfw"), 'edit_pages', $cloudfw_slider_slug, 'cloudfw_setup_slider', '', 702);
	}
...
function cloudfw_admmin_scripts(){
	...
	wp_enqueue_script('cloudfw-pack', TMP_ADMIN . '/js/pack.min.js', array(), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-ddmenu', TMP_ADMIN . '/js/ddmenu/menu.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-functions', TMP_ADMIN . '/js/functions.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-ui', TMP_ADMIN . '/js/cloudfw-ui.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-notifications', TMP_ADMIN . '/js/notifications.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-shortcode-generator', TMP_ADMIN . '/js/shortcode-generator.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-composer', TMP_ADMIN . '/js/composer.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-script', TMP_ADMIN . '/js/script.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-nav-menu', TMP_ADMIN . '/js/nav_menu.min.js', array('jquery'), cloudfw_get_combined_version());

	if ( is_theme_setting_page('slider') ) {
		wp_enqueue_script('cloudfw-slider-management', TMP_ADMIN . '/js/slider-management.min.js', array('jquery'), cloudfw_get_combined_version());
	}

	wp_enqueue_script('cloudfw-picker', TMP_ADMIN . '/js/colorpicker/js/colorpicker.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-picker-eye', TMP_ADMIN . '/js/colorpicker/js/eye.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-picker-utils', TMP_ADMIN . '/js/colorpicker/js/utils.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-fileinput', TMP_ADMIN . '/js/fileinput.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-fileuploader', TMP_ADMIN . '/js/fileuploader.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-zclip', TMP_ADMIN . '/js/zclip/jquery.zclip.min.js', array('jquery'), cloudfw_get_combined_version(), true);
	wp_enqueue_script('cloudfw-text-editor', TMP_ADMIN . '/js/markitup/jquery.markitup.min.js', array('jquery'), cloudfw_get_combined_version());
	wp_enqueue_script('cloudfw-asmselect', TMP_ADMIN . '/js/jquery.asmselect.min.js', array('jquery'), cloudfw_get_combined_version());
	if ( cloudfw_is_wpseo() ){
		wp_enqueue_script('cloudfw-seo', TMP_ADMIN . '/js/seo.min.js', array('jquery'), cloudfw_get_combined_version(), true);
	}
}

/**
 *	Load Media Scripts on Admin
 */
add_action('admin_enqueue_scripts', 'cloudfw_admin_scripts_media');
function cloudfw_admin_scripts_media(){
	global $pagenow;
...
<style type="text/css" media="screen">
	...
	#toplevel_page_CloudFW .wp-menu-image:before,
