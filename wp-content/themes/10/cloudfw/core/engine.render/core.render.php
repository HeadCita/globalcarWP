		...
		case 'module':
			...
			<?php
				...
				<?php elseif ($datas['layout'] == 'split'):
						...
						$id0 = isset($datas['data'][0]['id']) ? $datas['data'][0]['id'] : NULL;
						$id1 = isset($datas['data'][0]['id']) ? $datas['data'][0]['id'] : NULL;

						$datas['echo_title'][0] = '
								<label class="title" for="'. $id0 .'">'.
								_if($datas['title'][0], $datas['title'][0], __('Untitled Module','cloudfw')).
								_if( !( isset($datas['colon']) && !$datas['colon'] ), ':').'
								</label>
						';
						$datas['echo_title'][1] = '
								<label class="title" for="'. $id1 .'">'.
					...
					<?php
						...
						echo '
							<div class="module clean relative" style="background: transparent; border: 0; padding-bottom: 0;padding-right: 0;margin-right: -20px;">
								...
								<div class="grid threeof4 last">
									<div class="clear"></div>
									<a href="javascript:void(0);" id="" class="cloudfw-ui-modal-save-and-close small-button small-grey" style="float: right;"><span>'.__('Save','cloudfw').'</span></a>
								</div>
								<div class="clear"></div>
							</div>
						';
		...
		if ( isset( $data['description'] ) && $data['description'] )
			...
			if ( $data['version'] == 2 || !isset($data['version']) ) {
				admin_create_upload_button_v2(array(
					...
					'to_alt'              => isset($data['to_alt']) ? $data['to_alt'] : NULL,
					'reset'               => isset($data['reset']) ? $data['reset'] : NULL,
				));
