...
function cloudfw_fontface_render() {
	...
	if ( is_array($load_google_fonts) ) {
		foreach ($load_google_fonts as $font) {
			$font_name = preg_replace( '/\W+/', '-', strtolower( str_replace( ' ', '_', strip_tags( $font ) ) ) );
			if ( ! empty( $font_name ) ) {
				wp_register_style( strtolower( $font_name ), "//fonts.googleapis.com/css?family={$font}", array(), $version );			
				wp_enqueue_style ( strtolower( $font_name ) );			
			}
		}
	}
	...
	if (is_array($load_custom_fonts)) {
		...
		foreach ( $load_custom_fonts as $load_custom_fonts => $c_font ) {
			$font_name = preg_replace( '/\W+/', '-', strtolower( str_replace( ' ', '_', strip_tags( $custom_fonts[$c_font]["name"] ) ) ) );
			wp_register_style( $font_name, $custom_fonts[$c_font]["uri"], array(), $version );			
 ...
 *  @since 3.0
 */
function cloudfw_fontface_uri( $folder, $font, $is_child = false ){
