...
class WPT_Login {
	...
	private function __construct() {
		...
		add_action( 'show_user_profile', array( $this, 'user_meta_fields'), 1 );
		add_action( 'edit_user_profile', array( $this, 'user_meta_fields'), 1 );
				...
				if ( is_wp_error( $user ) ) {
					...
					if ( empty( $redirect ) ) {
						if ( ! empty( $_REQUEST['redirect_to'] ) ) {
							...
							$redirect = esc_url( $_REQUEST['_wp_http_referer'] );
	...
	public function process_registration() {
		...
		if ( ! empty( $_POST['wpt_register'] ) ) {
			...
			if ( $this->error_count() == 0 ) {
				...
				$use_wc_functions = cloudfw_check_onoff( 'custom_register', 'use_wc_functions' );

				if ( $password_method == 'via_user' ) {
					if ( function_exists('wc_create_new_customer') && $use_wc_functions ) {
						update_option( 'woocommerce_registration_generate_password', 'no' );
						$user_id = wc_create_new_customer( sanitize_email( $user_email ), wc_clean( $sanitized_user_login ), $password );
					} else {
					...
					if ( function_exists('wc_create_new_customer') && $use_wc_functions ) {
						...
						$user_id = wc_create_new_customer( sanitize_email( $user_email ), wc_clean( $sanitized_user_login ) );
					} else {
	...
	public function process_reset_password() {
		...
		if ( ! empty( $_POST['wpt_lost_password'] ) ) {
			...
			if ( !empty( $email_or_username ) && $to_level == 1 ) {
				...
				if ( $result ) {
					$this->add_message( cloudfw_translate( 'custom_login.widget.messages.check_mail' ) );
					$this->form_level = 'message';
				} else {
